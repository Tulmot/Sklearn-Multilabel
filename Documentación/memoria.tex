...
\begin{abstract}
Data mining is a area of computer science, which involves the analysis of large data sets to discover patterns. To do this you can use machine learning, it belongs to a subarea of computer science and the  artificial intelligence, the objective of machine learning is to develop techniques that allow machines to learn~\cite{wiki:datamining}.

In this project we will implement some multi-classifier algorithms (\textit{ensembles}), for Multi-Label on the Python library~\cite{python} Scikit-Learn~\cite{scikitlearn}. It has followed the Python style guide (PeP~\cite{pep}) and the convections that have been observed in Sklearn (Scikit-Learn). To make it better understand and more graphic, trees and graphs have been drawn, showing the results when executing those algorithms on some data sets. The algorithms we are going to focus on are Disturbing Neighbors~\cite{disturbingneighbors}, Random Oracles~\cite{randomoracles} and Rotation Forest~\cite{rotationforest}.
\end{abstract}
